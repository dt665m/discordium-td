...
use super::DreamView;
use game_shared::PresentationId;
use game_sim::dream::{EnemyKind, EssenceKind, RunPhase};

pub struct DreamScenePlugin;
impl Plugin for DreamScenePlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<DreamCamera>()
            .add_systems(Startup, setup_scene)
            .add_systems(Update, (sync_scene, animate_scene, follow_camera).chain())
            .add_systems(
                PostUpdate,
...
#[derive(Resource)]
pub struct DreamCamera {
    pub zoom: f32,
}
impl Default for DreamCamera {
    fn default() -> Self {
        Self { zoom: 1.0 }
    }
}
...
fn setup_scene(
    ...
    commands.init_resource::<DreamCamera>();
    ...
    let camera = commands
        .spawn((
            Camera3d::default(),
            DreamCameraRig { focus: Vec3::ZERO },
            ...
            Transform::from_xyz(0.0, 30.0, 23.0).looking_at(Vec3::ZERO, Vec3::Y),
            Msaa::Sample4,
        ))
        .id();
    #[cfg(not(target_arch = "wasm32"))]
...
fn follow_camera(
    view: Res<DreamView>,
    time: Res<Time>,
    settings: Res<DreamCamera>,
    mut cameras: Query<(&mut DreamCameraRig, &mut Transform, &mut Projection)>,
) {
    let h = &view.0.hero;
    for (mut rig, mut transform, mut projection) in &mut cameras {
        let position = world(h.position, 0.0);
        let aim = world(h.facing, 0.0) * 0.75;
        let target = if view.0.phase == RunPhase::Intro {
            Vec3::new(-4.0, 0.0, -6.0)
        } else {
            (position * 0.65 + aim).clamp(Vec3::new(-8.0, 0.0, -8.0), Vec3::new(8.0, 0.0, 8.0))
        };
        rig.focus = rig
            .focus
            .lerp(target, 1.0 - (-time.delta_secs() * 4.5).exp());
        let shake = h.hit_flash.min(0.2) * 0.45;
        let offset = Vec3::new(
            (time.elapsed_secs() * 91.0).sin() * shake,
            0.0,
            (time.elapsed_secs() * 79.0).cos() * shake,
        );
        *transform = Transform::from_translation(rig.focus + Vec3::new(0.0, 30.0, 23.0) + offset)
            .looking_at(rig.focus + offset, Vec3::Y);
        if let Projection::Orthographic(p) = &mut *projection {
            p.scale = settings.zoom.clamp(0.75, 1.4);
        }
    }
}
...
mod tests {
    use super::*;
    use game_sim::dream::DreamSimulation;
