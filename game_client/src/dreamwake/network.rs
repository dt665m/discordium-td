...
// Hosting launches the same dedicated game_server entry point; it never bypasses transport.
use super::{CapturedInput, DreamConnection, DreamPreferences, DreamView, Playtest};
use bevy::prelude::*;
use game_dream_net::*;
use game_sim::dream::{DreamInput, DreamSimulation, DreamSnapshot, RunPhase};
...
impl Plugin for DreamNetworkPlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<diagnostics::Telemetry>()
            .add_systems(
                Update,
                diagnostics::sample.before(game_client::network_tools::graphs::update_graphs),
            )
...
pub fn connect(world: &mut World, attempts: u32) {
    ...
    *world.resource_mut::<game_client::network_tools::graphs::DiagnosticHistory>() = default();
...
pub fn host(world: &mut World) {
    #[cfg(not(target_arch = "wasm32"))]
    {
        ...
        if !world.resource::<NetworkOptions>().hosted {
            ...
            let args = game_server::ServerArgs::parse_from([
            ...
            std::thread::Builder::new()
                ...
                .spawn(move || {
                    if let Err(error) = game_server::run(args) {
...
fn poll_network(world: &mut World) {
    ...
    if let Some((epoch, revision, ack_input, ack_action, snapshot)) = newest {
        ...
        for frame in runtime.pending.iter().take(18) {
