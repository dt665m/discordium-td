...
#[derive(Component)]
/// Panel root exposed so hosts can compose the existing controls into their UI.
pub struct ConditionerPanel;
...
fn scroll_panel(
    ...
    mut panels: Query<(&Interaction, &ComputedNode, &mut ScrollPosition), With<ConditionerPanel>>,
...
fn refresh_panel(
    state: Res<ConditionerDebug>,
    mut panels: Query<&mut Node, With<ConditionerPanel>>,
...
mod tests {
    ...
    fn structured_metrics_keep_directions_separate_and_labels_ascii() {
        ...
        assert!(
            world
                .query_filtered::<&BackgroundColor, With<ConditionerPanel>>()
                .iter(world)
        ...
        assert!(
            world
                .query_filtered::<&Node, With<ConditionerPanel>>()
                .iter(world)
