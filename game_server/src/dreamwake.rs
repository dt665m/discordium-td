...
use game_dream_net::{self as wire, DreamAction, DreamClientMessage, DreamServerMessage};
use game_sim::dream::{DreamInput, DreamSimulation, RunPhase};
use renet::{RenetServer, ServerEvent};

use crate::net::SharedNet;

const TICK: Duration = Duration::from_nanos(1_000_000_000 / game_sim::dream::TICK_HZ as u64);
...
impl DreamClock {
    ...
    fn advance(&mut self, now: Duration, mut step: impl FnMut()) -> DreamTick {
        let mut tick = DreamTick::default();
        let mut steps = 0;
        while now >= self.next_tick && steps < 6 {
            ...
            if self.ticks_since_snapshot == wire::TICKS_PER_SNAPSHOT {
...
pub(crate) fn run_dreamwake(shared: SharedNet, seed: u64, lucid: bool) -> Result<(), String> {
    let mut server = RenetServer::new(wire::connection_config());
    let mut authority = DreamAuthority::new(seed, lucid, shared.max_clients);
    let started = Instant::now();
    let mut previous = started;
    let mut clock = DreamClock::default();
    log::info!(
        "Dreamwake authority ready: seed={seed}, lucid={lucid}, max_players={}, simulation=60Hz, snapshots=20Hz",
        shared.max_clients
    );
    loop {
        let now = Instant::now();
        ...
        server.update(delta);
        {
            ...
            if let Err(error) = transport.update(delta, &mut server) {
                log::warn!("Dreamwake transport update: {error}");
            }
        }
            ...
            match event {
                ServerEvent::ClientConnected { client_id } => {
                    if shared.bootstrap.on_client_connected(client_id).is_ok()
                        && authority.admit(client_id)
                    {
                        log::info!(
                            "Dreamwake player {client_id} joined ({} travelers)",
                            authority.peers.len()
                        );
                        authority.revision = authority.revision.wrapping_add(1);
                        if let Some(state) = authority.state(client_id) {
                            server.send_message(
                                client_id,
                                wire::STATE_CHANNEL,
                                wire::encode(&state),
                ...
                ServerEvent::ClientDisconnected { client_id, reason } => {
                    ...
                    log::info!("Dreamwake player {client_id} left: {reason}");
                }
            }
        }
        receive_messages(&mut server, &mut authority, elapsed);
        let tick = clock.advance(elapsed, || authority.step(elapsed));
        if tick.snapshot_due {
            send_states(&mut server, &mut authority);
        }
        // Renet generates packets once per server tick, including ACKs/retries
        // when no new gameplay message is queued. The 1 ms receive poll below
        // must not also become a 1 kHz Renet packet-generation loop.
        if tick.send_due {
            shared
                .transport
                .lock()
...
mod tests {
    ...
    use game_sim::dream::{DreamSnapshot, EnemyKind, RewardKind, UpgradeKind};
    ...
    #[test]
    fn bootstrap_and_configured_limits_reject_unissued_or_ninth_player_and_host_is_elected() {
        let mut h = Harness::new();
        for _ in 0..9 {
            h.join();
        }
