...
fn test_hero(sim: &mut DreamSimulation, action: impl FnOnce(&mut Hero)) {
    let mut query = sim.world.query::<&mut Hero>();
...
fn memory(sim: &mut DreamSimulation, kind: MemoryKind, essence: Option<EssenceKind>) {
    test_hero(sim, |hero| {
        hero.view.position = [0.0, 0.0];
        hero.view.facing = [0.0, -1.0];
        hero.view.memories[0] = MemorySlot::new(kind);
        hero.view.memories[0].essence = essence;
...
fn basic_combo_has_direction_healing_and_memory_refresh() {
    ...
    test_hero(&mut sim, |h| {
        ...
        h.view.hp = 100.0;
        h.view.combo = 2;
        h.view.memories[0].cooldown = 3.0;
    });
    sim.step(DreamInput {
        action_sequences: [0; 5],
        attack: true,
...
fn twin_echo_vast_and_haste_change_cast_behavior() {
    ...
    cast(&mut vast);
    assert_eq!(vast.snapshot().state.projectiles[0].pierce, 4);
...
fn frost_status_damage_synergy_and_leech_heal() {
    ...
    test_hero(&mut sim, |h| {
        h.view.memories[0].cooldown = 0.0;
    });
    cast(&mut sim);
...
fn dash_protects_from_a_committed_warning() {
    ...
    for mut target in sim.world.query::<&mut Enemy>().iter_mut(&mut sim.world) {
        target.view.windup = DT * 0.5;
        target.view.target = [0.0; 2];
...
fn rest_recovers_health_and_spends_shards() {
    ...
    assert!(sim.snapshot().hero.hp > 140.0);
...
fn action_ids(snapshot: &DreamSnapshot, sequence: u32) -> Vec<game_shared::PresentationId> {
...
fn world_ring_ids_use_actor_and_encounter_state_instead_of_wall_clock_tick() {
    ...
    if let Some(mut first) = initial.world.get_mut::<Enemy>(targets[0].1) {
        first.view.windup = DT * 0.5;
        first.view.target = [0.0; 2];
        first.view.warn_radius = 2.0;
    }
    if let Some(mut second) = initial.world.get_mut::<Enemy>(targets[1].1) {
        second.view.hp = 0.0;
    }
