...
fn enter_room(world: &mut World) {
    ...
    world.resource_scope(|world, mut run: Mut<Run>| {
        ...
        let room = run.room;
        let mut hero_query = world.query::<&mut Hero>();
        ...
        for (index, mut hero) in party.into_iter().enumerate() {
            ...
            hero.view.hp = (hero.view.hp + hero.view.max_hp * 0.10).min(hero.view.max_hp);
        ...
        if room == 3 || room == 7 {
            ...
            run.message =
                "Restored 40% health. Spend 45 shards to refine a Memory, then choose a blessing."
                    .into();
            for mut hero in world.query::<&mut Hero>().iter_mut(world) {
                hero.view.hp = (hero.view.hp + hero.view.max_hp * 0.4).min(hero.view.max_hp);
            }
...
pub(super) fn reconcile_ready(world: &mut World) -> bool {
    ...
    let mut query = world.query::<&Hero>();
...
pub(super) fn continue_run(world: &mut World, id: u64) -> bool {
    ...
    {
        let mut query = world.query::<&mut Hero>();
...
pub(super) fn upgrade(hero: &mut Hero, kind: UpgradeKind) {
    match kind {
        ...
        UpgradeKind::Health => {
            hero.view.max_hp += 50.0;
            hero.view.hp = (hero.view.hp + 75.0).min(hero.view.max_hp);
        }
        UpgradeKind::Defense => {
...
pub(super) fn recalculate_cooldowns(hero: &mut Hero) {
    for slot in &mut hero.view.memories {
