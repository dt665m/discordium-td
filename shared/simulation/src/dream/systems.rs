...
fn action_effect(
    ...
    commands.spawn((
        DreamOwned,
        Effect(game_shared::PresentationInstance {
...
fn heal(hero: &mut Hero, amount: f32) {
    if hero.view.hp <= 0.0 {
        return;
    }
    hero.view.hp = (hero.view.hp + amount).min(hero.view.max_hp);
}
...
fn hit_enemy(
    ...
    let dealt = amount.min(enemy.view.hp);
    enemy.view.hp -= amount;
...
fn hit_hero(commands: &mut Commands, run: &mut Run, hero: &mut Hero, damage: f32) {
    if hero.dodge_left > 0.0 || hero.dash_left > 0.0 || hero.view.hp <= 0.0 {
        return;
    }
    ...
    hero.view.hp = (hero.view.hp - (amount - absorbed)).max(0.0);
...
pub(super) fn begin_tick(
    ...
    for mut hero in &mut heroes {
        decrement(&mut hero.view.attack_cooldown);
        decrement(&mut hero.view.dash_cooldown);
        decrement(&mut hero.dash_left);
        decrement(&mut hero.dodge_left);
        decrement(&mut hero.movement_lock);
        decrement(&mut hero.shield_left);
        ...
        for slot in &mut hero.view.memories {
            decrement(&mut slot.cooldown);
        }
    }
    for mut enemy in &mut enemies {
        decrement(&mut enemy.recovery);
        decrement(&mut enemy.slow_left);
...
pub(super) fn age_transients(
    ...
    for (entity, mut value) in &mut effects {
        value.0.age_ticks += 1;
        if value.0.age_ticks >= value.0.duration_ticks {
...
fn execute_memory(
    ...
    run: &mut Run,
    hero: &mut Hero,
    enemies: &mut Query<(Entity, &mut Enemy)>,
    kind: MemoryKind,
    essence: Option<EssenceKind>,
    power: f32,
    origin: [f32; 2],
    ...
    for (_, entity) in ordered {
        ...
        if cast.remaining <= 0.0 {
            if let Some(mut hero) = heroes
                .iter_mut()
                .find(|h| h.view.id == cast.owner && h.view.hp > 0.0)
            {
                execute_memory(
                    &mut commands,
                    &mut run,
...
pub(super) fn wisp_actions(
    ...
    for (_, entity) in ordered {
        ...
        let target = enemies.iter().filter(|e| e.view.hp > 0.0).min_by(|a, b| {
...
pub(super) fn enemy_actions(
    ...
    for (_, entity) in ordered {
        ...
        if kind == EnemyKind::Boss {
            let phase = if enemy.view.hp < enemy.view.max_hp * 0.34 {
                3
            } else if enemy.view.hp < enemy.view.max_hp * 0.68 {
                2
            } else {
                1
            };
    ...
    for center in support_pulses {
        for (_, mut enemy) in &mut enemies {
            ...
            {
                enemy.view.hp = (enemy.view.hp + enemy.view.max_hp * 0.09).min(enemy.view.max_hp);
            }
        }
    }
}
...
pub(super) fn projectile_actions(
    ...
    for (_, entity) in ordered {
        ...
        let from = bolt.view.position;
        bolt.view.position = add(from, scale(bolt.view.direction, bolt.speed * DT));
        bolt.lifetime -= DT;
        ...
        if bolt.view.friendly {
            ...
            let mut targets: Vec<_> = enemies
                .iter()
                .filter(|(_, e)| e.view.hp > 0.0 && !bolt.hit_ids.contains(&e.view.id))
...
pub(super) fn finish_encounter(
    ...
    for mut hero in &mut heroes {
        ...
        hero.ready = false;
        if hero.view.hp <= 0.0 {
            hero.view.hp = hero.view.max_hp * 0.5;
        }
    }
