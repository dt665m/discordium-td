...
use super::{DreamConnection, DreamPreferences, DreamView};
            ...
            HudText::Companion(index) => companions
                ...
                .map(|hero| {
                    color.0 = if hero.hp <= 0.0 {
                        ...
                        super::scene::companion_color(hero.id)
                    };
                    if hero.hp <= 0.0 {
    ...
    for (kind, mut node, mut background) in &mut fills {
        let fraction = match *kind {
            ...
            Fill::Companion(index) => companions
                .get(index)
                .map(|h| {
                    background.0 = super::scene::companion_color(h.id);
...
fn intro_panel(
    ...
    _snap: &game_sim::dream::DreamSnapshot,
) {
...
fn reward_panel(
    commands: &mut Commands,
    root: Entity,
    snap: &game_sim::dream::DreamSnapshot,
    selected: usize,
) {
...
fn transition_panel(commands: &mut Commands, root: Entity, snap: &game_sim::dream::DreamSnapshot) {
...
mod tests {
    ...
    fn ui_app() -> App {
        ...
        .add_plugins((
            DreamUiPlugin,
            super::super::typography::DreamTypographyPlugin,
        ));
        app
    }
