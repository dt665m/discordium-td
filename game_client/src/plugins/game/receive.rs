...
pub(super) fn network_update(
        ...
        match game_shared::decode::<ReliableServerMessage>(&bytes) {
            Ok(ReliableServerMessage::JoinSnapshot(snapshot)) => {
                acknowledge_pending_moves(
                    ...
                    snapshot.world.last_move_seq_for(snapshot.you),
                );
            ...
            Ok(msg) => {
                ...
                if let Some(meta) = delta.sim_meta {
                    if meta.match_epoch != runtime_inner.match_epoch
                        || delta.phase != MatchPhase::InProgress
                    {
                ...
                // RTT measurement: match the local movement ACK to recorded send times
                if let Some(ack_seq) = delta.last_move_seq_for(runtime_inner.client_id) {
                ...
                acknowledge_pending_moves(
                    &mut runtime_inner.pending_moves,
                    delta.last_move_seq_for(runtime_inner.client_id),
                );
    ...
    // Reconstruction and event observation above consume every snapshot, but
    // restore/replay is needed only for the newest accepted authority this frame.
    if accepted_snapshot {
        if let Some(latest) = snapshot_buffer.snapshots.back() {
            if let Some(meta) = latest.world.sim_meta {
                reconcile_local_sim(
                    &mut local_sim,
                    &mut smoothing,
                    &latest.world,
    ...
    // Use the newest received batch to discipline the presentation clock once
    // per frame. Advancing before receive adds an avoidable frame of starvation.
    snapshot_buffer.advance_render_time(wall_time, dt_secs);
}
