...
impl DreamSimulation {
    ...
    pub fn new(seed: u64, lucid: bool) -> Self {
        ...
        app.world_mut().spawn((DreamOwned, Hero::default()));
    ...
    pub fn snapshot_for(&self, player_id: u64) -> DreamSnapshot {
        ...
        for (hero, enemy, projectile, wisp, effect, number, delayed) in self
            .snapshot_query
            .lock()
        ...
        {
            if let Some(v) = hero {
                saved.heroes.push(v.clone());
            }
            if let Some(v) = enemy {
                saved.enemies.push(v.clone());
            }
        ...
        saved
            .effects
            .sort_by_key(|v| (v.0.id.owner, v.0.id.action_seq, v.0.id.slot));
        ...
        DreamSnapshot {
            ...
            hero: local.view.clone(),
            heroes: saved.heroes.iter().map(|h| h.view.clone()).collect(),
            ready: local.ready,
    ...
    pub fn add_player(&mut self, id: u64) -> bool {
        ...
        let mut query = self.world.query::<&Hero>();
        ...
        hero.dodge_left = 1.5;
        self.world.spawn((DreamOwned, hero));
    ...
    fn resize_party(&mut self, before: usize, after: usize) {
        ...
        for mut enemy in self.world.query::<&mut Enemy>().iter_mut(&mut self.world) {
            enemy.view.hp *= factor;
            enemy.view.max_hp *= factor;
        }
    }
    ...
    pub fn swap_memories_for(&mut self, id: u64, a: usize, b: usize) -> bool {
        ...
        let mut query = self.world.query::<&mut Hero>();
