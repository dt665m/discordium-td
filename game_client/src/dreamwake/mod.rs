...
use game_sim::dream::{DreamInput, DreamSimulation, DreamSnapshot, RunPhase, TICK_HZ};
use scene::{DreamCamera, DreamCameraRig};
...
pub fn run() {
    ...
    App::new()
        ...
        .add_plugins((
            game_client::network_tools::NetworkToolsPlugin,
            scene::DreamScenePlugin,
            ui::DreamUiPlugin,
...
fn startup_options() -> (u64, Playtest, network::NetworkOptions, DreamConnection) {
    ...
    let mut connection = DreamConnection {
        http_base: std::env::var("TD_DREAM_HTTP_BASE")
...
fn startup_options() -> (u64, Playtest, network::NetworkOptions, DreamConnection) {
    let seed = fresh_seed();
    let mut base = option_env!("TD_WEB_HTTP_BASE")
...
fn keyboard_actions(
    ...
    if keys.just_pressed(KeyCode::Equal) || keys.just_pressed(KeyCode::NumpadAdd) {
        camera.zoom = (camera.zoom - 0.1).max(0.75);
    }
    if keys.just_pressed(KeyCode::Minus) || keys.just_pressed(KeyCode::NumpadSubtract) {
        camera.zoom = (camera.zoom + 0.1).min(1.4);
    }
    for pad in &gamepads {
...
fn apply_ui_actions(world: &mut World) {
    use game_dream_net::DreamAction;
        ...
        if let Some(cursor) = window.cursor_position() {
            if let Ok(ray) = camera.viewport_to_world(transform, cursor) {
                    ...
                    ray.intersect_plane(Vec3::ZERO, InfinitePlane3d::new(Vec3::Y))
                {
                    ...
                    aim = (Vec2::new(point.x, point.z) - Vec2::from_array(view.0.hero.position))
                        .normalize_or_zero();
                }
            }
        }
