...
fn edit_hero(sim: &mut DreamSimulation, id: u64, edit: impl FnOnce(&mut Hero)) {
...
fn enemies_target_living_players_and_area_attacks_hit_the_party() {
    ...
    for mut e in sim.world.query::<&mut Enemy>().iter_mut(&mut sim.world) {
        e.recovery = 0.0;
    }
    sim.step_multiplayer(&[]);
...
fn fallen_teammates_cannot_act_and_revive_only_after_shared_clear() {
    ...
    for id in [1, 22] {
        edit_hero(&mut doomed, id, |h| h.view.hp = 0.0);
    }
